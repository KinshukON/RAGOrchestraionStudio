...
> **Version:** v2.2 (Sprints 1–6 complete — commercial legibility enhancements across catalog, advisor, executive summary, observability, and environments)
...
| Architecture Catalog | `modules/architecture-catalog/` | `ArchitectureCatalogPage.tsx` (empty state CTA → seeds + refetches), **`RequiredIntegrationsPanel`** (live connector health per arch), `architecture-catalog.css`. **Sprint 6:** catalog cards show **tier badge** (core/advanced/specialized), **business outcome badge**, **commercial strip** (setup days · cost tier · governance posture), and **"why it wins"** callout |
| Architect Advisor | `modules/architecture-catalog/` | `ArchitectAdvisor.tsx` (**8-question wizard** → recommendation card with operational profile + required integrations panel; handles LCW / fine-tuning alternatives). **Sprint 6:** 3 new questions (budget sensitivity, latency tolerance, explainability), **Commercial Profile** grid (setup effort, cost posture, governance, ROI lever), **"Why Not the Alternatives?"** rejection list |
| Arch Profiles | `modules/architecture-catalog/` | `archProfiles.ts` — central source of truth for per-arch required integrations, operational complexity, cost tier, commercial use cases. **Sprint 6:** extended with `businessOutcome`, `governancePosture`, `benchmarkIdentity`, `recommendedFor`, `tier`, `whyItWins` for all 18 architectures |
...
| Environments | `modules/environments/` | `EnvironmentsPage.tsx` (readiness score pill + animated fill bar per card, 3-step promotion bar, EmptyState component). **Sprint 6:** 5-point **deployment readiness scorecard**, contextual **blocker messages** (⚠️ missing integrations, 💡 governance next step), **cost connection link** → Environment Cost Heatmap. `environments.css` |
| Governance | `modules/governance/` | `GovernancePage.tsx` |
| Observability | `modules/observability/` | `ObservabilityPage.tsx` (**7 tabs**: Operations, Retrieval Quality, Governance Risk, Cost Analytics, Run History, Audit Log, 🤖 AI Recommendations). **Sprint 6:** AI Recommendations upgraded to **prescriptive**: operating metrics row (cost/run, quality compliance %, critical issues), **"Fix this →"** action buttons mapped by recommendation type, expandable **"Why?"** explanations. **`TraceExplorer.tsx`** (per-node latency bars, expandable detail panels, `trace-explorer.css`), `observability.css` |
| Executive Summary | `modules/executive-summary/` | `ExecutiveSummaryPage.tsx` (**4 tabs**: 📊 Overview — live KPIs from `/executive/kpis` + health/quality, 🎯 Action Board, 💰 ROI Summary, 📋 Business Case Generator). **Sprint 6:** **Platform headline banner** (state-derived status + best-fit architecture + top action CTA), **3-variant Business Case selector** (💼 Executive / ⚙️ Technical / 🛡️ Governance) with variant-specific content framing. `executive-summary.css` |
...
| **Sprint 6** | **Commercial Legibility (Novelty-First)** — 5-phase upgrade: (P1) `archProfiles.ts` extended with 6 commercial fields (`businessOutcome`, `governancePosture`, `benchmarkIdentity`, `recommendedFor`, `tier`, `whyItWins`) for all 18 architectures; catalog cards enriched with tier badge, business outcome badge, commercial strip, "why it wins" callout. (P2) Architect Advisor upgraded to 8-question decision assistant with `whyNotOthers` rejection list and `commercialProfile` grid (setup effort, cost posture, governance, ROI lever). (P3) Executive Summary: platform headline banner (state-derived status + best-fit arch + top action CTA) + 3-variant Business Case selector (Executive/Technical/Governance). (P4) Observability AI Recommendations upgraded to prescriptive: operating metrics row, "Fix this →" action buttons, expandable "Why?" panels. (P5) Environments: 5-point deployment readiness scorecard, contextual blocker messages, cost connection link. |
